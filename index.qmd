...
::: small-text
::: {.column width="50%"}
![](0_Figures/1_TOC_Example_1_1.png)
:::

::: {.column width="50%"}
Teniendo en cuenta el límite de 2,000 $\frac{min}{semana}$:

![](0_Figures/1_TOC_Example_1_2.png)
:::
:::
:::
...
## Production Engineering
![](0_Figures/2_ProductionEngineering.png)

## Production Engineering
![](0_Figures/2_ProductionEngineering2.png)

## Sistemas de ensamble
![](0_Figures/2_AssemblyLine.jpg)

### Clasificación
![](0_Figures/2_AssemblySystems1.png)

### Tipos de problemas de ensamble
![](0_Figures/2_AssemblyProblems.png)

### Assembly Sequence Planning
![](0_Figures/2_AssemblySequencePlanning.png)

### Assembly Sequence Planning
![](0_Figures/2_AssemblySequencePlanning2.png)
...
$$
\begin{align*}
\textbf{Takt Time} &= \frac{\text{Available Production Time}}{\text{Demanda per Time}}
\\
\end{align*}
$$
...
::: r-fit-text
```{r}
titulos <- c("Reglas", "Descripción")
X1 <- c("Ranked Following Tasks", "Ranked Positional Weight", "Shortest Processing Time", "Longest Processing Time")
X2 <- c("Se asigna primero la operación con mayor numero de tareas siguientes.", "Se asigna primero la operación con mayor tiempo posicional.", "De las tareas candidatas, se asigna la de menor tiempo de
procesamiento.", "De las tareas candidatas, se asigna la de mayor tiempo de procesamiento.")
...
```{r}
titulos <- c("TASK", "PREDECESSOR TASK", "TASK TIME [s]")
X1 <- c("A", "B", "C", "D", "E", "F", "G", "H", "I", "J", "K")
X2 <- c("-", "A", "B", "-", "D", "C", "C", "E", "E", "F-G-H-I", "J")
X3 <- c(15, 50, 60, 45, 30, 12, 65, 10, 8, 15, 5)
...
1. Diagrama de red de tareas
2. Sumar los $t_{i}$
3. Calcular takt time
4. Graficar estado actual contra takt time
5. Calcular la cantidad de estaciones (redondeando siempre al valor entero superior más cercano):
$$
\begin{align*}
\textbf{CE} &\geq \frac{\sum_{i \in Tareas}^{} t_{i}}{\text{Takt Time}}
\\
\end{align*}
$$
6. Asignar tareas con una heurística, siempre respetando la holgura y precedencias, calculando el Remaining Time, el tiempo de ciclo $S_{j}$ de cada estación y su idle time.
7. Graficar la nueva distribución de tareas por estación contra el takt time.
8. Calcular la capacidad y eficiencia de la línea.
...
::: small-text
\begin{align*}
\text{min } Z &= \sum_{j \in Estaciones} Y_{j} \\

\textbf{s.t.} \\
\sum_{i \in Tareas} t_{i} \times X_{i,j} &\leq C \times Y_{j} \qquad ~\forall ~j \in \text{Estaciones} \\
\sum_{j \in Estaciones} X_{i,j} &= 1 \qquad ~\forall ~i \in \text{Tareas} \\
\sum_{j \in Estaciones} j \times X_{k,j} &\leq \sum_{j \in Estaciones} j \times X_{i,j} \qquad ~\forall ~k \prec ~i \\
Y_{j+1} &\leq Y_{j} \qquad ~\forall ~j \in \text{Estaciones} \\

\textbf{Donde:} \\
\textbf{\(X_{i,j}\)} &= \text{1 si la tarea \(i\) se realiza en la estación \(j\),} \\
&\text{0 lo contrario.} \\
\textbf{\(Y_{j}\)} &= \text{1 si la estación \(j\) se abre, 0 lo contrario.} \\
\textbf{\(t_{i}\)} &: \text{Tiempo de procesamiento de la tarea \(i\).} \\
\textbf{\(C\)} &: \text{Tiempo de ciclo.} \\
\textbf{\(k\)} &: \text{Tarea que precede a la tarea \(i\).} \\
...
1. Diagrama de red de tareas
2. Calcular el tiempo de ciclo (sin redondear)
$$
\begin{align*}
\textbf{Cycle Time} &\geq \frac{\sum_{i \in Tareas}^{} t_{i}}{\text{Workstations quantity}}
\\
\end{align*}
$$
3. Graficar estado actual contra el cycle time ideal
4. Asignar tareas con una heurística, siempre respetando la holgura y precedencias, calculando el Remaining Time, el tiempo de ciclo $S_{j}$ de cada estación y su idle time (no se permiten valores negativos).
5. Graficar la nueva distribución de tareas por estación contra el tiempo de ciclo calculado.
6. Calcular la capacidad y eficiencia de la línea.
...
\textbf{s.t.} \\
\sum_{j \in Estaciones} X_{i,j} &= 1  \qquad ~\forall ~i \in \text{Tareas} \\
\sum_{i \in Tareas} t_{i} \times X_{i,j} &\leq C \qquad ~\forall ~j \in \text{Estaciones} \\
\sum_{j \in Estaciones} j \times X_{k,j} &\leq \sum_{j \in Estaciones} j \times X_{i,j} \qquad ~\forall ~k \prec ~i \\

\textbf{Donde:} \\
\textbf{\(X_{i,j}\)} &= \text{1 si la tarea \(i\) se realiza en la estación \(j\), 0 lo contrario.} \\
\textbf{\(t_{i}\)} &: \text{Tiempo de procesamiento de la tarea \(i\).} \\
\textbf{\(C\)} &: \text{Tiempo de ciclo.} \\
\textbf{\(k\)} &: \text{Tarea que precede a la tarea \(i\).} \\
...
::: small-text
\begin{align*}
\text{min } Z &= C \times \sum_{j \in Estaciones}^{} Y_{j} \\
...
::: small-text
\begin{align*}
\text{min } Z &= \sum_{j \in Estaciones}^{} Y_{j} + W_{j}\\

\textbf{s.t.} \\
\sum_{j \in Estaciones} X_{i,j} &= 1 \qquad ~\forall ~i \in \text{Tareas} \\
\sum_{i \in Tareas} t_{i} \times X_{i,j} &\leq C \times Y_{j} \times W_{j} \qquad ~\forall ~j \in \text{Estaciones} \\
\sum_{j \in Estaciones} j \times X_{k,j} &\leq \sum_{j \in Estaciones} j \times X_{i,j} \qquad ~\forall ~k \prec ~i \\
Y_{j+1} &\leq Y_{j} \qquad ~\forall ~j \in \text{Estaciones}\\ 
...
&\text{0 lo contrario.} \\
\textbf{\(Y_{j}\)} &= \text{1 si la estación \(j\) se abre, 0 lo contrario.} \\
\textbf{\(\text{W_max}_{j}\)} &: \text{Cantidad máxima de operarios que pueden} \\
&\text{ser asignados a la estación \(j\).} \\
\textbf{\(t_{i}\)} &: \text{Tiempo de procesamiento de la tarea \(i\).} \\
\textbf{\(C\)} &: \text{Tiempo de ciclo.} \\
\textbf{\(k\)} &: \text{Tarea que precede a la tarea \(i\).}\\
...
$$
\begin{align*}
T_{k} = \sum_{j \in Modelos} C_{k,j} \times d_{j}
\end{align*}
$$
:::
...
\begin{align*}
\textbf{Donde:} \\
d_{j} &: \text{es la participación del modelo j} \\
&\text{en el mix de la demanda.} \\
C_{k,j} &: \text{es la duración de la tarea k para} \\
&\text{el modelo j.} \\
\end{align*}
$$
:::
...
### MALBP
![](0_Figures/2_MALBP2.png)

### MALBP

1. Plantear el diagrama de red de tareas de cada modelo.
2. Plantear el diagrama compuesto de red de tareas.
3. Calcular los pesos de cada referencia del producto.
4. Calcular el takt time.
5. Calcular el tiempo promedio ponderado de cada tarea.
6. Calcular la cantidad de estaciones usando la suma de los tiempos y el takt time.
7. Balancear la línea aplicando SALBP.

### MALBP
![](0_Figures/2_MALBP3.png)
